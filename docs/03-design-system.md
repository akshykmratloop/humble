...
Playful, confident, slightly chaotic, premium enough to trust with a real photo and real conversations, social/screenshot-worthy. Humor targets the *mechanic* (mutual rejection), never a person's appearance, identity, or protected class.
...
- `rejection` (Reject / Humble Match accent) `#6C5CE7` (chosen deliberately *not* red — red reads as "error"; Humble Match is a celebration, so it borrows game-purple, distinct from the coral "like" primary and from the red-adjacent tones used for destructive/error states)
...
- `PowerUpCard` / `InventoryGrid` *(post-MVP)*
...
- **GameInventory** *(post-MVP)*: tasteful, not "mobile casino" — a small grid, not a slot-machine aesthetic.
